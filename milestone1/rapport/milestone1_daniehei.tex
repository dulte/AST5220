...
H = \frac{\dot{a}}{a},
\end{equation}
where $a$ is the scale factor and $\dot{a}$ its time derivative. The Hubble parameters is an observable quantity which describes the expansion of the Universe. From this we are going to calculate the \textit{conformal time} $\eta$, which is a convenient way of representing time in many of the equations we are going to solve in later milestones. The physical interpretation of $\eta$ is that given that the expansion of the Universe is frozen, $\eta$ is the time it takes light to travel from us to the outer limit of the observable universe. This means that for a given time, $\eta$  gives the proper distance to the particle horizon with the relation $d_{part. hor.} = c\eta$.
